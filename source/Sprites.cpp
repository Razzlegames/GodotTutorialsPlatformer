//******************************************************************************
/**
*  Used to control all apsects of sprites in this game. Including memory access
*   availability linked list
*/
#include "Sprites.h"
#include "Debug.h"
...
u8 Sprites::createSprite(int x, int y, int sprite_size_code, int color)
{
    ...
    setSpriteColorAndShape(spriteTypeToShape(sprite_size_code),color, sprite_number);
...
//   iprintf("\x1b[0;0H Pallet_mem: 0x%x pal_byte_size: %d: pal: %x", SPRITE_PALETTE,size,pallet);
//    int test_index = 0;
//    iprintf("\x1b[1;0H *Pallet_mem[%d] = {0x%04x%04x..}",test_index,SPRITE_PALETTE[test_index], SPRITE_PALETTE[test_index+1]);
//    iprintf("\x1b[5;0H *pallet[%d] = {0x%04x%04x..}",test_index,pallet_16[test_index], pallet_16[test_index+1]);
}
//******************************************************************************
/**
...
bool Sprites::setGFXMemoryUsed(int begin_index, unsigned int size)
{
    ...
    do
    {
        ...
        else if(current->next == NULL)
        {
            ...
            new_gfx->next = NULL;
            // Since all our work is done, return (and give result of bounds check)
...
bool Sprites::freeGFXBlock(int begin_index)
{
    ...
    while(current != NULL)
    {
        // If the block to delete is found, arrange links and delete this block
        if(current->begin_index = begin_index)
        {
            if(last != NULL)
            {    
