...
int BallBlue::pallet_tile_mem_loaded = 0;

/// GFX memory index (how far into GFX memory are the 
///  tiles for this character: if using same 
///  tile set for all gummys)
int BallBlue::gfx_memory_index = 0;

BallBlue::BallBlue(int pallet_number2): Ball(pallet_number2)
{
...
//**************************************************************************
void BallBlue::init(u8 pallet_number, 
        ...
        int velocity_vector_x, int velocity_vector_y)
{
    ...
    Character::init((unsigned char*)ball_blue_Tiles,BALL_TILE_SIZE,
    ...
    // Check if tile memory has already been initialized for 
    // this sprite type

    if(!pallet_tile_mem_loaded)
    {

        gfx_memory_index = Sprites::appendTileGFX(
                (unsigned char*)ball_blue_Tiles, 
    ...
    Sprites::setTileIndex(byteIndexToTileIndex(gfx_memory_index)*2, 
            sprite_index);
