        ...
        int velocity_vector_x, int velocity_vector_y)
{
    ...
    // Set up Collision Box (assume middle of box 
    //          is current position of ball)
    Vector2D offset(0,5);
    collision_box.bottom_left = Vector2D(0,0)+offset;
    collision_box.top_right = Vector2D(7,10)+offset;

    Character::init((unsigned char*)heart_16x16_Tiles,
            sizeof(heart_16x16_Tiles),
    ...
    if(!pallet_tile_mem_loaded)
    {
        ...
        gfx_memory_index = Sprites::appendTileGFX(
                (unsigned char*)heart_16x16_Tiles, 
