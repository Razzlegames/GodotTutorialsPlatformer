...
{ 
	        ...
	        c2.top_right == Vector2D(top_right.x,bottom_left.y))
	{
	  ...
	  (Vector2D(c2.bottom_left.x,c2.top_right.y)== Vector2D(bottom_left.x,top_right.y)) ||
	   Vector2D(c2.bottom_left.x,c2.top_right.y) == top_right || 
	   Vector2D(c2.bottom_left.x,c2.top_right.y) == Vector2D(top_right.x,bottom_left.y))
	{
		return true;
	}
	else if(Vector2D(c2.top_right.x,c2.bottom_left.y) == bottom_left || 
	  (Vector2D(c2.top_right.x,c2.bottom_left.y)  == Vector2D(bottom_left.x,top_right.y)) ||
	   Vector2D(c2.top_right.x,c2.bottom_left.y)  == top_right || 
	   Vector2D(c2.top_right.x,c2.bottom_left.y)  == Vector2D(top_right.x,bottom_left.y))
	{
		return true;
	}
	return false;
